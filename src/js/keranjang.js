...
document.addEventListener('click', function(e) {
    ...
    if (button && button.closest('.break-inside-avoid.group')) {
        ...
        const priceElement = Array.from(card.querySelectorAll('h3')).find(el => el.innerText.includes('Rp'));
        const price = priceElement ? parseInt(priceElement.innerText.replace('Rp', '').replace('.', '').trim()) : 0;
